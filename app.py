...
def read_and_flatten_json(json_path):
    ...
    flattened_data.columns = [f"make_model_trim_{
        col}" for col in flattened_data.columns]
...
def clean_and_rename_columns(raw_data, flattened_data):
    ...
    all_cars = pd.concat([raw_data.reset_index(drop=True),
                         flattened_data.reset_index(drop=True)], axis=1)
...
def add_derived_columns(all_cars):
    ...
    all_cars['horsepower_per_100_dollars'] = (
        all_cars['horsepower_hp'] / all_cars['msrp']) * 100
...
all_cars['msrp_qcut'] = pd.qcut(all_cars['msrp'], q=5, labels=[
                                'Q1', 'Q2', 'Q3', 'Q4', 'Q5'])

# Existing code for creating the string labels
quantile_ranges = all_cars.groupby('msrp_qcut', observed=True)[
    'msrp'].agg(['min', 'max']).reset_index()
quantile_labels = [f"${row['min']:, .0f} to ${
    row['max']:, .0f}" for index, row in quantile_ranges.iterrows()]
# Create a dictionary to map 'msrp_qcut' labels to string labels
quantile_dict = {qcut: label for qcut, label in zip(
    ['Q1', 'Q2', 'Q3', 'Q4', 'Q5'], quantile_labels)}
...
app.layout = dbc.Container([
    ...
    dbc.Row(
        [
            dbc.Col(
                html.Img(src='https://github.com/car-api-team/docs/blob/main/carapi.png?raw=true',
    ...
    dbc.Row([
        dbc.Col(
            dbc.Card(
                ...
                # Add a slight box-shadow for depth
...
def update_box_graph_with_markers(engine_type_selected, price_range_selected, make_selected, fuel_type_selected):
    ...
    if engine_type_selected:
        df_to_plot = df_to_plot[df_to_plot['engine_type'].isin(
            engine_type_selected)]

    if price_range_selected:
        df_to_plot = df_to_plot[df_to_plot['msrp_label'].isin(
    ...
    if fuel_type_selected:
        df_to_plot = df_to_plot[df_to_plot['fuel_type'].isin(
            fuel_type_selected)]
    ...
    filtered_df_agg_make_median = df_to_plot.groupby('make', observed=True)[
        'horsepower_per_100_dollars'].median().reset_index()
    filtered_df_agg_make_median_sort_list = filtered_df_agg_make_median.sort_values(
        'horsepower_per_100_dollars')['make'].tolist()
    ...
    figScatter.update_layout(
        ...
        yaxis=dict(
            categoryorder='array',
            # Reverse the list as I swapped axes
            categoryarray=filtered_df_agg_make_median_sort_list[::-1],
            title_text=''
        ),
        xaxis=dict(
            title_text=''
